...
const Connect = ({setIsConnected}) => {
    ...
    return (
        ...
        <section className="block static" >
            ...
            <button onClick={() => setIsOpenModal(!isOpenModal)} className="block w-full bg-main-pink text-main-white hover:bg-main-white border transition-all hover:text-main-pink hover:border hover:border-main-pink font-medium rounded-lg text-sm px-5 py-2.5 text-center" type="button">
              Connexion administrateur
            </button>

            <div tabIndex="-1" aria-hidden="true" 
            className={`fixed -top-[90vh] mx-auto inset-x-0 !-inset-[70vh] right-0 z-[999] ${ isOpenModal == false ? "hidden" : "" } 
            w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[70vh] 
            `}>
                ...
                <div className="relative w-full max-w-md max-h-full inset-0 mx-auto">
                    <div className="relative bg-main-white rounded-lg shadow-[50px] border-2 border-main-pink">
                        <button type="button" onClick={() => setIsOpenModal(!isOpenModal)} className="absolute top-3 right-2.5 text-gray-800 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="authentication-modal">
