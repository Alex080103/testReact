...
const GestionJeune = ({isOpenStep, setIsOpenStep, hours, clickedDate, modalAction, rdvDate, endDate, actualDate, setClickedDate, action}) => {
        ...
        <div className="p-2">
            ...
            <MultiSteps isOpenStep={isOpenStep} numberOfSteps={4}  content={[
              ...
              <form onSubmit={ (event) => handleSubmit(event, "justify")} className="grid p-2">
                ...
                { action == "conseiller" ? <p className="mt-4 mb-2 text-center text-xl italic">Avec {rdvDate[1]},<br></br> <strong>le {rdvDate[0].toLocaleDateString()}</strong></p>
                ...
                {action == "conseiller" ? 
                <label for="justification" className="ml-4 text-lg sm:text-xl italic underline mb-2 mt-4 text-center">Mail envoyé à {rdvDate[1]}</label>
                :
              ...
              <form className="grid p-2">
                ...
                { action !=="conseiller" ? <p className="mb-4 mt-2 italic md:text-lg text-center">Une réponse vous seras fait par mail et sera visible sur votre espace personnel en cas de validation.</p> : ""}
                <div className="flex flex-col md:flex-row gap-2  mt-4">
                  <button onClick={() => setIsOpenStep(isOpenStep => isOpenStep - 1)} className="bg-main-pink font-bold w-full md:w-1/2 py-2 text-main-white border mx-auto hover:bg-main-white hover:text-main-pink hover:border transition-all hover:border-main-pink rounded-lg" >
                    Retour
                  </button>
            ...
            : modalAction == "delete" ? 
            <form onSubmit={ (event) => handleSubmit(event, "justify")} className="grid p-2">
                <p className="text-3xl mt-8 md:text-4xl font-title text-center sm:mt-4">Supprimer le Rendez Vous</p>
                { action == "conseiller" ? <p className="mt-4 mb-2 text-center text-xl italic">Avec {rdvDate[1]},<br></br> <strong>le {rdvDate[0].toLocaleDateString()}</strong></p>
                : null }
                {action == "conseiller" ? 
                <label for="justification" className="ml-4 text-lg sm:text-xl italic underline mb-2 mt-4 text-center">Mail envoyé à {rdvDate[1]}</label>
