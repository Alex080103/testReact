...
const CardTeam = ({content}) => {
    ...
    return (
        <div className="flex-col mx-auto flex h-[400px] md:h-[500px] w-[310px] md:w-[350px] lg:w-[380px] relative">
            <div className={`h-3/4 bg-[url('${content.picture}')] bg-cover w-[310px] md:w-[350px] lg:w-[380px] bg-[center_top_-2.3rem] rounded-tr-[15%] rounded-t-xl relative`}>
            </div>
            <div className="h-1/4 text-center border-2 border-gray-800 flex flex-wrap content-center justify-center  border-t-0 rounded-b-xl">
                <h4 className="font-extrabold text-[28px] w-[100%]">{content.user_surname} <span className="uppercase italic">{content.user_name}</span></h4>
            ...
            <div className={`absolute bottom-1/4 transition-all duration-[0.9s] ease-out ${ showText == false ? "translate-y-1/4 opacity-0" : "opacity-1" }`}>
                <div className="bg-gray-800 text-main-white">
                    <p className="p-4 text-sm md:text-xl">{content.user_description}</p>
                </div>
            </div>
        </div>
    )
