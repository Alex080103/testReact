...
class Post 
{
    ...
    public function __construct( int $id, string $date, string $description, string $link, string $video_link, mixed $poster, mixed $video)
    {
        $this->id = $id;
        $this->date = $date;
...
class PostRepository extends ConnectBdd
{
    function showAllPosts()
    {
        ...
        foreach ( $datas as $data ) {
            ...
            $data['post_link'], $data['post_video_link'], $data['post_poster'], $data['post_video']);
    ...
    function addPost(object $post): bool
    {
        ...
        $req = $this->bdd->prepare("INSERT INTO post (post_date, post_description, post_link, post_video_link, post_poster, post_video)
         VALUES (CURRENT_TIMESTAMP, ?, ?, ?, ?, ?)");
        $postIsAdd = $req->execute([$description, $link, $video_link, $poster, $video]);
        return $postIsAdd;
    }
